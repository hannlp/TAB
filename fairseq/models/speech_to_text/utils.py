...
grads = {}
def save_grad(name, log=True):
    def hook(grad):
        grads[name] = grad
        if log:
            metrics.log_scalar("{}-g-norm".format(name), grads[name].norm(), round=3)
            metrics.log_scalar("{}-g-mean".format(name), grads[name].abs().mean(), round=3)
            metrics.log_scalar("{}-g-var".format(name), grads[name].abs().var(), round=3)
    return hook

def save_to_dict(encoder_out, encoder_padding_mask, ctc_out: dict={}):
    encoder_out = {
