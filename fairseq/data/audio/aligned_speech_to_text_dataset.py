...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __getitem__(
        ...
        return index, source, target, aligned_target, transcript
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        if self.src_dict is not None and self.src_texts is not None:
            ...
            transcript_ntokens = sum(t.size(0) for _, _, _, _, t in samples)
        else:
            transcript = None
...
class SpeechToTextDatasetCreator(object):
    ...
    def _from_list(
        ...
        audio_paths, n_frames, src_texts, tgt_texts, aligned_tgt_texts, ids = [], [], [], [], [], []
